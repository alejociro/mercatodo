<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ trans('admin.products.titles.create a product') }}
        </h2>
    </x-slot>
                    ...
                    <div class="md:flex items-center mt-12">
                        ...
                        <div class="w-full md:w-1/2 flex flex-col md:ml-6 md:mt-0 mt-4">
                            ...
                            <input id="price" name="price"
                                   class="leading-none text-gray-900 p-3 focus:outline-none focus:border-blue-700 mt-4 bg-gray-100 border rounded border-gray-200"
                                   @class(['form-control', 'is-invalid' => $errors->has('price')]) value="{{ old('price') }}"
                                   type="text" required/>
                            @error('price')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="md:flex items-center mt-12">
                        <div class="w-full md:w-1/2 flex flex-col">
                            ...
                            <input id="stock" name="stock"
                                   class="leading-none text-gray-900 p-3 focus:outline-none focus:border-blue-700 mt-4 bg-gray-100 border rounded border-gray-200"
                                   @class(['form-control', 'is-invalid' => $errors->has('stock')]) value="{{ old('stock') }}"
                                   type="text" required/>
                            @error('stock')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
